...
int ATTClass::handleNotify(uint16_t handle, const uint8_t* value, int length)
{
  ...
  for (int i = 0; i < ATT_MAX_PEERS; i++) {
    ...
    // SGC PATCH (see SGC_PATCHES.md #3): do not discard sendAclPkt result —
    // a failed send (bounded busy-poll timeout) must surface as writeValue()
    // returning 0 so the caller can detect the stuck link and abort cleanly.
    if (HCI.sendAclPkt(_peers[i].connectionHandle, ATT_CID, notificationLength, notification) == 0) {
      numNotifications++;
    }
  }
