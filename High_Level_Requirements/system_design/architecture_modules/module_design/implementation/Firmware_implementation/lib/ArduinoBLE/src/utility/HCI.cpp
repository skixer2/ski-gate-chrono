...
{
  // SGC PATCH (see SGC_PATCHES.md #1): bounded busy-wait.
  // Original spins forever when the peer stops ACKing (phone controller
  // stall) and on disconnect (_pendingPkt never cleared) — freezing the
  // main loop, starving the watchdog, and hanging the device.
  uint32_t sgc_t0 = millis();
  while (_pendingPkt >= _maxPkt) {
    poll();
    if ((uint32_t)(millis() - sgc_t0) > 2000) {
      return -1;  // TX queue stuck — drop packet, report failure
    }
  }
...
void HCIClass::handleEventPkt(uint8_t /*plen*/, uint8_t pdata[])
{
  ...
  if (eventHdr->evt == EVT_DISCONN_COMPLETE)
  {
    ...
    // SGC PATCH (see SGC_PATCHES.md #2): controller flushes TX queue on
    // disconnect — clear host-side pending count so the sendAclPkt
    // busy-poll exits instead of hanging forever.
    _pendingPkt = 0;
