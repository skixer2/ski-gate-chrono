...
bool LittleFSStorage::begin() {
    ...
    /* Gemini-tuned: block_cycles=100 reduces aggressive compaction,
       lookahead=32 saves RAM. Avoids v2 root dir compaction corruption. */
    auto* fs = new LittleFileSystem2("littlefs", NULL, 4096, 100, 256, 32);
...
void LittleFSStorage::erase_all() {
    ...
    /* Erase raw sectors directly — no LittleFS operations.
       fs->unmount/reformat can HardFault on corrupt metadata.
       Boot init handles fresh format+mount via auto-reformat. */
    if (m_bd) {
        auto* raw = static_cast<mbed::BlockDevice*>(m_bd);
